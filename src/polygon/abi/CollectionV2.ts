...
export const events = {
    AddItem: event("0x1464db75caec2c4f1b56b5246ef01a229ff5edc3b0fcf9b2953fec53b3987d8f", {"_itemId": indexed(p.uint256), "_item": p.struct({"rarity": p.string, "maxSupply": p.uint256, "totalSupply": p.uint256, "price": p.uint256, "beneficiary": p.address, "metadata": p.string, "contentHash": p.string})}),
    ...
    Complete: event("0x01b7dcb42d49142a99e4c98da755263c600213a33b780986779405b9823501d3", {}),
    CreatorshipTransferred: event("0xf25c6226b06a6bf4b2c2391bb66962bb4290c323cf8b5814efb185aba3f42d5f", {"_previousCreator": indexed(p.address), "_newCreator": indexed(p.address)}),
    Issue: event("0x57e2fe3f4d57a597e655a893f43c395d7dc18cb9c9c1898eda75cc0ad7608525", {"_beneficiary": indexed(p.address), "_tokenId": indexed(p.uint256), "_itemId": indexed(p.uint256), "_issuedId": p.uint256, "_caller": p.address}),
    ...
    RescueItem: event("0x87a972ab2db2d47a0bbefe72cefc4fe5a38b1b9d2bc4b9f366b59fdb6dbd9581", {"_itemId": indexed(p.uint256), "_contentHash": p.string, "_metadata": p.string}),
    SetApproved: event("0x9ea3c7bd2b8ff87a3768286a529f5932ca385584c8ddf42eea8eade2ac1fb121", {"_previousValue": p.bool, "_newValue": p.bool}),
    SetEditable: event("0xc8ca126810b77bfdcb10ae82ef3bc72619d404f9e3c4bf2057f03f057dcf230a", {"_previousValue": p.bool, "_newValue": p.bool}),
    SetGlobalManager: event("0x68bb2af98a57ecc88afb707f2d75e1e253c3e30191071548355731e21322a432", {"_manager": indexed(p.address), "_value": p.bool}),
    SetGlobalMinter: event("0x27defe4b442489e757345f4f743096c5db8d2399e1474bb9653118cdf5313869", {"_minter": indexed(p.address), "_value": p.bool}),
    SetItemManager: event("0x1271b13cf757efd2a177fe5f26c2111b69ce904f1d0e3372832dc4e4773298cc", {"_itemId": indexed(p.uint256), "_manager": indexed(p.address), "_value": p.bool}),
    SetItemMinter: event("0x45a634291439a8ec187e2b70279696615889c34c17c3a1208a7f85544e13b565", {"_itemId": indexed(p.uint256), "_minter": indexed(p.address), "_value": p.uint256}),
    ...
    UpdateItemData: event("0xc3835f7dd5063bbc8fdc7077944498159008d73d30bf55fe6513e683c8c38209", {"_itemId": indexed(p.uint256), "_price": p.uint256, "_beneficiary": p.address, "_metadata": p.string}),
}

export const functions = {
    COLLECTION_HASH: fun("0x7e12acce", {}, p.bytes32),
    ISSUED_ID_BITS: fun("0x950fa904", {}, p.uint8),
    ITEM_ID_BITS: fun("0x5ec7c537", {}, p.uint8),
    MAX_ISSUED_ID: fun("0xf6f07b63", {}, p.uint216),
    MAX_ITEM_ID: fun("0x48622018", {}, p.uint40),
    addItems: fun("0x425f60ed", {"_items": p.array(p.struct({"rarity": p.string, "price": p.uint256, "beneficiary": p.address, "metadata": p.string}))}, ),
    ...
    batchTransferFrom: fun("0xf3993d11", {"_from": p.address, "_to": p.address, "_tokenIds": p.array(p.uint256)}, ),
    completeCollection: fun("0xf59d9e3f", {}, ),
    createdAt: fun("0xcf09e0d0", {}, p.uint256),
    creator: fun("0x02d05d3f", {}, p.address),
    decodeTokenId: fun("0x7efd9112", {"_id": p.uint256}, {"itemId": p.uint256, "issuedId": p.uint256}),
    domainSeparator: fun("0xf698da25", {}, p.bytes32),
    editItemsData: fun("0x0ebde06e", {"_itemIds": p.array(p.uint256), "_prices": p.array(p.uint256), "_beneficiaries": p.array(p.address), "_metadatas": p.array(p.string)}, ),
    encodeTokenId: fun("0xebd46d64", {"_itemId": p.uint256, "_issuedId": p.uint256}, p.uint256),
    ...
    globalManagers: fun("0x7682dfca", {"_0": p.address}, p.bool),
    globalMinters: fun("0xb4c2025e", {"_0": p.address}, p.bool),
    initImplementation: fun("0x30127574", {}, ),
    initialize: fun("0xd14fee77", {"_name": p.string, "_symbol": p.string, "_baseURI": p.string, "_creator": p.address, "_shouldComplete": p.bool, "_isApproved": p.bool, "_rarities": p.address, "_items": p.array(p.struct({"rarity": p.string, "price": p.uint256, "beneficiary": p.address, "metadata": p.string}))}, ),
    isApproved: fun("0x28f371aa", {}, p.bool),
    isApprovedForAll: fun("0xe985e9c5", {"owner": p.address, "operator": p.address}, p.bool),
    isCompleted: fun("0xfa391c64", {}, p.bool),
    isEditable: fun("0xb4da4e37", {}, p.bool),
    isInitialized: fun("0x392e53cd", {}, p.bool),
    isMintingAllowed: fun("0x2b481883", {}, p.bool),
    issueTokens: fun("0x7c8f76a1", {"_beneficiaries": p.array(p.address), "_itemIds": p.array(p.uint256)}, ),
    itemManagers: fun("0xbe4763b3", {"_0": p.uint256, "_1": p.address}, p.bool),
    itemMinters: fun("0x6d67b1ab", {"_0": p.uint256, "_1": p.address}, p.uint256),
    items: fun("0xbfb231d2", {"_0": p.uint256}, {"rarity": p.string, "maxSupply": p.uint256, "totalSupply": p.uint256, "price": p.uint256, "beneficiary": p.address, "metadata": p.string, "contentHash": p.string}),
    itemsCount: fun("0xe2c03ace", {}, p.uint256),
    ...
    rescueItems: fun("0x3c963655", {"_itemIds": p.array(p.uint256), "_contentHashes": p.array(p.string), "_metadatas": p.array(p.string)}, ),
    safeBatchTransferFrom: fun("0x28cfbd46", {"_from": p.address, "_to": p.address, "_tokenIds": p.array(p.uint256), "_data": p.bytes}, ),
    ...
    setApproved: fun("0x46d5a568", {"_value": p.bool}, ),
    setBaseURI: fun("0x55f804b3", {"_baseURI": p.string}, ),
    setEditable: fun("0x2cb0d48a", {"_value": p.bool}, ),
    setItemsManagers: fun("0x239a603b", {"_itemIds": p.array(p.uint256), "_managers": p.array(p.address), "_values": p.array(p.bool)}, ),
    setItemsMinters: fun("0xe6170a53", {"_itemIds": p.array(p.uint256), "_minters": p.array(p.address), "_values": p.array(p.uint256)}, ),
    setManagers: fun("0x3bdcc923", {"_managers": p.array(p.address), "_values": p.array(p.bool)}, ),
    setMinters: fun("0x41bceced", {"_minters": p.array(p.address), "_values": p.array(p.bool)}, ),
    ...
    transferCreatorship: fun("0x6d2e4b1b", {"_newCreator": p.address}, ),
